...
var Bullet = Class(
{
	...
	Bullet: function(team)
	{
		...
		// 创建子弹精灵
		//
		m_sptBul = new Sprite("res/Misc.png", 8, 8);
		m_sptBul.Hide();
		m_sptBul.SetZ(Const.Z_BULL);
		App.GameUI.Append(m_sptBul);

		// 子弹所属的队伍
		m_team = team;

		// 小型爆炸
		m_boom = new Boom(false);
	},
	...
	Update: function()
	{
		...
		case BullState.FIRE:			// -发射状态
			for(var i = 0; i < $Speed; ++i)
			{
				...
				//
				case HitState.HIT:
					m_boom.Start($X - 28, $Y - 28);
					m_state = BullState.BOOM;
					return;
				...
				// 更新子弹位置
				m_sptBul.Move(Const.POS_X + $X, Const.POS_Y + $Y);
			}
			break;
	...
	_Go: function()
	{
		...
		for(i = 0; i < Const.MAX_TANK; ++i)
		{
			...
			// 跳过同一队伍的
			//
			if(tank.Team == m_team || !tank.IsLive())
				continue;

			//
			// 子弹与坦克的碰撞
			//
			if(tank.CheckColl(m_sptBul))
			{
			...
			for(j = 0; j < tank.BulMax; j++)
			{
				...
				{
					if(bul._sptBul.CollidesWith(m_sptBul))
					{
						// 对家的子弹也消失
	...
	_TileHit: function(b1, b2, col, row, p1, p2, q1, q2)
	{
		...
		if(b1 & q1)				// 后面块
		{
			 ...
			 *    才可以打后面块。
			 */
			if($Pow || (hit == 0 && b2 != Const.BLOCK_IRON))
			{
				b1 &= ~q1;
				if(b1 &= q2)	// 同方向扩散
					b1 &= ~q2;
